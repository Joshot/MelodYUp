...
export async function POST(req) {
  try {
    ...
    if (!chordData || !Array.isArray(chordData))
      return NextResponse.json({ error: 'No chord data' }, { status: 400 })
    ...
    const { data: saved, error: dbErr } = await supabase.from('songs').insert({
      ...
      title: title||'Untitled', key_note: key||'C', key_scale: scale||'major',
      bpm: bpm||120, chords: uniqueChords, chord_data: chordData,
      total_duration: totalDuration||0, total_beats: totalBeats||0,
    }).select().single()
