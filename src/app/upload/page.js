...
const dummyAnalysis = {
  key_note: 'C', key_scale: 'major', bpm: 120,
  chords: ['C', 'Am', 'F', 'G', 'C', 'Em', 'F', 'G'], capo: 0,
}
...
export default function UploadPage() {
  ...
  const isValidUrl = (u) => /(?:youtube\.com\/watch\?v=|youtu\.be\/)[\w-]{11}/.test(u)
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from('songs').insert({
      user_id: user.id,
      title: videoTitle || 'Lagu dari YouTube',
      youtube_url: url,
          ...
          {steps.map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition ${
                step > i + 1 ? 'bg-green-500 text-white' : step === i + 1 ? 'bg-violet-600 text-white' : 'bg-[#1e1e2e] text-[#64748b]'
              }`}>{step > i + 1 ? '✓' : i + 1}</div>
              <span className={`text-sm hidden sm:block ${step === i + 1 ? 'text-white font-semibold' : 'text-[#64748b]'}`}>{s}</span>
              {i < steps.length - 1 && <div className={`w-8 h-0.5 ${step > i + 1 ? 'bg-green-500' : 'bg-[#1e1e2e]'}`} />}
            </div>
          ))}
        </div>
        <div className="w-full max-w-2xl">
          {step === 1 && (
            <div className="bg-[#12121a] border border-[#1e1e2e] rounded-2xl p-8">
              ...
              {getYouTubeId(url) && isValidUrl(url) && (
                <div className="flex gap-4 items-center bg-[#1e1e2e] rounded-xl p-3 mb-4">
                  <Image src={`https://img.youtube.com/vi/${getYouTubeId(url)}/mqdefault.jpg`}
              ...
              <button onClick={handleAnalyze} disabled={!isValidUrl(url)}
          ...
          {step === 2 && (
            <div className="bg-[#12121a] border border-[#1e1e2e] rounded-2xl p-8 text-center">
              <div className="w-20 h-20 rounded-full bg-violet-600/10 border-2 border-violet-500/30 flex items-center justify-center mx-auto mb-6 animate-pulse">
              ...
              <div className="bg-[#1e1e2e] rounded-full h-2 overflow-hidden mb-2">
                <div className="h-full bg-gradient-to-r from-violet-600 to-cyan-500 rounded-full transition-all duration-300" style={{ width: `${progress}%` }} />
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <button onClick={() => { setStep(1); setUrl(''); setResult(null) }}
                  className="flex-1 bg-transparent border border-violet-600 text-violet-400 hover:bg-violet-600/10 rounded-xl py-3 font-semibold transition">Analisis Ulang</button>
